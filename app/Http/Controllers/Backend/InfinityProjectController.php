...
class InfinityProjectController extends Controller
{

    public function index()
    {
        if (!\auth()->user()->ability('superAdmin', 'manage_infinity,infinity_project')) {
            return redirect('admin/index');
        }

        $projects = Project::whereType('project')->whereShowIn('infinity_project')->orderBy('id' ,'desc')->paginate(10);
        $head = 'Infinity Project';
        $route = '.infinity.projects.';
    ...
    public function store(ProjectRequest $request)
    {
        ...
        $input['show_in']   = 'infinity_project';
        $input['type']      = 'project';

        if ($image = $request->file('image')) {
            $filename = time() . md5(uniqid()) .'.'.$image->getClientOriginalExtension();
            $path = ('public/assets/infinity_project/' . $filename);
            $path_data = ('assets/infinity_project/' . $filename);
        ...
        return redirect()->route('admin.infinity.projects.index')->with([
            'message' => 'Infinity Project Created Successfully',
            'alert-type' => 'success'
        ]);
    }
    ...
    public function update(ProjectRequest $request, Project $project)
    {
        ...
        return redirect()->route('admin.infinity.projects.index')->with([
            'message' => 'Infinity Project Updated Successfully',
            'alert-type' => 'success'
        ]);
    }
    ...
    public function destroy(Project $project)
    {
        ...
        return redirect()->route('admin.infinity.projects.index')->with([
            'message' => 'Infinity Project Deleted Successfully',
            'alert-type' => 'success'
        ]);
