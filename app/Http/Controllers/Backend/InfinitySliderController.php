...
class InfinitySliderController extends Controller
{

    public function index()
    {
        if (!\auth()->user()->ability('superAdmin', 'manage_infinity,infinity_slider')) {
            return redirect('admin/index');
        }

        $projects = Project::whereType('slider')->whereShowIn('infinity_slider')->orderBy('id' ,'desc')->paginate(10);
        $head = 'Infinity Slider';
        $route = '.infinity.sliders.';
    ...
    public function store(ProjectRequest $request)
    {
        ...
        $input['show_in']   = 'infinity_slider';
        $input['type']      = 'slider';

        if ($image = $request->file('image')) {
            $filename = time() . md5(uniqid()) .'.'.$image->getClientOriginalExtension();
            $path = ('public/assets/infinity_slider/' . $filename);
            $path_data = ('assets/infinity_slider/' . $filename);
        ...
        return redirect()->route('admin.infinity.sliders.index')->with([
            'message' => 'Infinity Slider Created Successfully',
            'alert-type' => 'success'
        ]);
    }
    ...
    public function update(ProjectRequest $request, Project $slider)
    {
        ...
        return redirect()->route('admin.infinity.sliders.index')->with([
            'message' => 'Infinity Slider Updated Successfully',
            'alert-type' => 'success'
        ]);
    }
    ...
    public function destroy(Project $slider)
    {
        ...
        return redirect()->route('admin.infinity.sliders.index')->with([
            'message' => 'Infinity Slider Deleted Successfully',
            'alert-type' => 'success'
        ]);
